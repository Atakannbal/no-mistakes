...
func TestPRStep_AttributionDisabled_OmitsPipelineSection(t *testing.T) {
	t.Parallel()
	dir, baseSHA, headSHA := setupGitRepo(t)
	...
	if strings.Contains(ghLog, "## Pipeline") {
		t.Fatalf("expected no Pipeline section with attribution disabled, got:\n%s", ghLog)
	}
	if strings.Contains(ghLog, "no-mistakes") {
		t.Fatalf("expected no no-mistakes attribution in PR body, got:\n%s", ghLog)
	}
	if !strings.Contains(ghLog, "## Risk Assessment\n\n⚠️ Medium: touches critical error handling") {
		t.Fatalf("expected risk assessment section to still be present, got:\n%s", ghLog)
	}
}
...
// TestPRStep_AutoDetectsGitHubTemplate_RendersConfiguredLayout mirrors
// TestPRStep_CustomTemplate_RendersConfiguredLayout but with no pr.template
// configured: the repo-standard .github/pull_request_template.md should be
// auto-detected and used the same way an explicitly configured template
// would be.
func TestPRStep_AutoDetectsGitHubTemplate_RendersConfiguredLayout(t *testing.T) {
	t.Parallel()
	dir, baseSHA, headSHA := setupGitRepo(t)
	...
	if !strings.Contains(ghLog, "# fix: improve pipeline header UX (feature/PROJ-123-improve-header)") {
		t.Fatalf("expected auto-detected template heading, got:\n%s", ghLog)
	}
	if !strings.Contains(ghLog, "Jira: PROJ-123") {
		t.Fatalf("expected Jira ticket placeholder filled in from branch name, got:\n%s", ghLog)
	}
	...
	if strings.Contains(ghLog, "## Pipeline") || strings.Contains(ghLog, "## Risk Assessment") {
		t.Fatalf("expected built-in section headings to be absent under an auto-detected template, got:\n%s", ghLog)
	}
}
